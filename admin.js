...
async function init() {
  if (!SUPABASE_READY) {
    loginError.textContent =
      'Supabase n’est pas encore configuré. Ajoute les clés dans supabase-config.js.';
    return;
  }
...
async function loadProducts() {
  ...
  productsEl
    ...
    .forEach(b => {
      b.onclick = () => deleteProduct(b.dataset.delete);
    });
...
async function saveProduct(e) {
  ...
  if (
    ...
    formMessage.textContent =
      'Nom, prix et photo sont requis pour une nouvelle carte.';
   ...
   * UPLOAD DE LA PHOTO
   *
   * Le nom original de la photo n'est jamais utilisé.
   * On crée un nom propre avec crypto.randomUUID().
   */
  if (file) {
    const ext = (
...
async function loadReservations() {
  const {
    ...
    .from('reservations')
    .select(
      'id,buyer_name,buyer_phone,status,created_at,products(name,price)'
    )
    .order('created_at', {
      ascending: false
    });
